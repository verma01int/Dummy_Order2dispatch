...
export default function SOCheckPage({ user, orders, updateOrders }) {
  const [selectedOrders, setSelectedOrders] = useState([])
  const [activeTab, setActiveTab] = useState("pending")
  const [searchTerm, setSearchTerm] = useState("")

  // Filter orders based on user role and those with expected delivery date
  const getFilteredOrders = () => {
    let filtered = orders.filter((order) => order.expectedDeliveryDate)
    if (user.role !== "master") {
  ...
  const pendingOrders = filteredOrders.filter((order) => !order.soChecked)
  const historyOrders = filteredOrders.filter((order) => order.soChecked)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">SO Check</h1>
        <p className="text-gray-600">Review and approve sales orders</p>
      </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50 hover:bg-gray-50 border-b border-gray-200">
                      ...
                      {activeTab === "history" && (
                        <TableHead className="font-semibold text-gray-900 py-4 px-6">SO Checked Date</TableHead>
                      )}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {displayOrders.map((order) => (
                      <TableRow key={order.id} className="hover:bg-gray-50 transition-colors border-b border-gray-100">
                        ...
                        {activeTab === "history" && (
                          <TableCell className="py-4 px-6">
                            <Badge className="bg-blue-500 text-white rounded-full">{order.soCheckedDate}</Badge>
                          </TableCell>
                        )}
                      </TableRow>
                    ))}
