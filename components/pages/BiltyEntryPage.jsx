...
export default function BiltyEntryPage({ user, orders, updateOrders }) {
  ...
  // Filter orders based on user role and wetman entry completed
  const getFilteredOrders = () => {
    let filtered = orders.filter((order) => order.wetmanEntryCompleted)
    if (user.role !== "master") {
  ...
  const pendingOrders = filteredOrders.filter((order) => !order.biltyEntryCompleted)
  const historyOrders = filteredOrders.filter((order) => order.biltyEntryCompleted)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Bilty Entry</h1>
      </div>

      <Card className="border-0 shadow-lg">
        <CardHeader>
          <CardTitle>Bilty Entry Management</CardTitle>
        </CardHeader>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-2xl">
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>Bilty Entry</CardTitle>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      type="file"
                      accept="image/*"
                      onChange={(e) => setFormData((prev) => ({ ...prev, biltyImage: e.target.files[0] }))}
                    />
                  </div>
                </div>
