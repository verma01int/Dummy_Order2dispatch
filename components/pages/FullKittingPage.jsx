...
export default function FullKittingPage({ user, orders, updateOrders }) {
  ...
  // Filter orders based on user role and bilty entry completed
  const getFilteredOrders = () => {
    let filtered = orders.filter((order) => order.biltyEntryCompleted)
    if (user.role !== "master") {
  ...
  const pendingOrders = filteredOrders.filter((order) => !order.fullKittingCompleted)
  const historyOrders = filteredOrders.filter((order) => order.fullKittingCompleted)
  ...
  const handleSubmit = () => {
    ...
    const updatedOrders = orders.map((order) => {
      if (order.id === selectedOrder.id) {
        return {
          ...order,
          fullKittingCompleted: true,
          fullKittingDate: new Date().toISOString().split("T")[0],
          ...formData,
        }
      }
      return order
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Full Kitting</h1>
      </div>

      <Card className="border-0 shadow-lg">
        <CardHeader>
          <CardTitle>Full Kitting Management</CardTitle>
        </CardHeader>
                  ...
                  <TableBody>
                    {displayOrders.map((order) => (
                      <TableRow key={order.id} className="hover:bg-gray-50 transition-colors border-b border-gray-100">
                        ...
                        {activeTab === "history" && (
                          <TableCell className="py-4 px-6">
                            <Badge className="bg-orange-500 text-white rounded-full">{order.typeOfBill}</Badge>
                          </TableCell>
                        )}
                      </TableRow>
                    ))}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-2xl">
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>Full Kitting</CardTitle>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setFormData((prev) => ({ ...prev, typeOfBill: value }))}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Select" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="Common Bill">Common Bill</SelectItem>
